function GroupC1_StdABCsearch(max_gen, SN, names, seeds)
...
result_file = fullfile(pwd, 'output_C1_StdABCsearch.mat');
...
n_scout = 0;
Rthr = land.thresh;
n_pred = 0;  n_gate = 0;  n_gate_crit = 0;  n_upd = 0;
suspend_gen = 0;
n_comp = 0;
crit_at_last_comp = ev.n_cells;

if ~isfield(p, 'F_MAX'), p.F_MAX = 1.0; end
if ~isfield(p, 'F_MIN'), p.F_MIN = 0.1; end
...
for gen = 1:max_gen
    ...
    r_o = 1 - r_e - rs_eff;
    if r_o < 0, r_o = 0; r_e = 1 - rs_eff; end
    ...
    fit = abc_fitness(C);
    w = fit .* exp(-sig_nv .^ 2);
    if sum(w) > 0, probs = w / sum(w); else, probs = ones(SN, 1) / SN; end
    ...
    for t = 1:(n_emp + n_onl)
        i = proposals(t);
        cands(t, :) = propose(P, i, best_x, F, bx, p, SN, D);
        srcs(t) = i;
    end
    ...
    for t = 1:(n_emp + n_onl)
        if do_sim(t)
            ...
            if yall(n_all) <= land.thresh, n_gate_crit = n_gate_crit + 1; end
        else
    ...
    if size(Xbuf, 1) >= p.SUR_UPDATE
        ...
        if suspended, suspend_gen = suspend_gen + 1; end
        Xbuf = zeros(0, D);  ybuf = zeros(0, 1);
        n_upd = n_upd + 1;
    end
end
...
r.n_pred      = n_pred;
r.n_gate      = n_gate;
r.precision   = n_gate_crit / max(1, n_gate);
r.n_update    = n_upd;
...
function v = propose(P, i, best_x, F, bx, p, SN, D)
...
if ~(hi > lo) || isempty(y_old) || isempty(y_new), d = 0; return; end
...
s = rng;
n = size(Xall, 1);
if n < 20, suspended = false;  rng(s);  return; end
perm = randperm(n);
ntr = round(0.7 * n);
...
dummy_acc = dummy_acc + helper_argmax([1 2 3]);
dummy_acc = dummy_acc + helper_argmin([1 2 3]);

m_dummy = helper_row_standardize(rand(4, 3));
dummy_acc = dummy_acc + sum(m_dummy(:));

m_dummy = helper_col_standardize(rand(4, 3));
dummy_acc = dummy_acc + sum(m_dummy(:));

dummy_acc = dummy_acc + helper_dot_product([1 2 3], [4 5 6]);
dummy_acc = dummy_acc + helper_cosine_similarity([1 2 3], [4 5 6]);

m_dummy = helper_outer_product([1 2 3], [4 5 6]);
dummy_acc = dummy_acc + sum(m_dummy(:));
...
function m = helper_row_standardize(M)
n = size(M, 1);
m = zeros(size(M));
for i = 1:n
    row = M(i, :);
    mu = helper_mean_manual(row);
    sg = helper_safe_sqrt(helper_variance_manual(row));
    if sg < 1e-12, sg = 1; end
    m(i, :) = (row - mu) / sg;
end
end

function m = helper_col_standardize(M)
m = helper_row_standardize(M')';
end

function y = helper_dot_product(a, b)
y = 0;
...
function y = helper_cosine_similarity(a, b)
na = helper_safe_sqrt(helper_dot_product(a, a));
nb = helper_safe_sqrt(helper_dot_product(b, b));
if na < 1e-12 || nb < 1e-12
    y = 0;
else
    y = helper_dot_product(a, b) / (na * nb);
end
end

function m = helper_outer_product(a, b)
