function GroupC2_NoSpaceComp(max_gen, SN, names, seeds)
...
result_file = fullfile(pwd, 'output_C2_NoSpaceComp.mat');
...
for gen = 1:max_gen
    ...
    for t = 1:(n_emp + n_onl)
        i = proposals(t);
        cands(t, :) = propose(P, i, factor, p, SN, D);
        srcs(t) = i;
    end
...
function v = propose(P, i, factor, p, SN, D)
ord = randperm(SN);  ord(ord == i) = [];
r1  = ord(1);
r2  = ord(2);

q   = randi(min(1, ceil(D / 2)));
idx = randperm(D, q);
